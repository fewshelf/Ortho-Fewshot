...
from models.util import create_model, get_teacher_name, orth_dist, deconv_orth_dist
...
def parse_option():
    ...
    parser.add_argument('--dataset', type=str, default='miniImageNet', choices=['miniImageNet', 'tieredImageNet',
                                                                                'CUB_Birds', 'Stanford_cars', 'Stanford_dogs'])
    ...
    parser.add_argument('--lambda', type=float, default=0.01, help='weight for orthogonal regularization')
...
def train(epoch, train_loader, module_list, criterion_list, optimizer, opt):
    ...
    for idx, data in enumerate(train_loader):
        ...
        diff += deconv_orth_dist(model_s.layer1[0].conv1.weight, stride=1) + deconv_orth_dist(
            model_s.layer1[0].conv3.weight, stride=1)

        diff += deconv_orth_dist(model_s.layer2[0].conv1.weight, stride=1) + deconv_orth_dist(
            model_s.layer2[0].conv3.weight, stride=1)

        diff += deconv_orth_dist(model_s.layer3[0].conv1.weight, stride=1) + deconv_orth_dist(
            model_s.layer3[0].conv3.weight, stride=1)

        diff += deconv_orth_dist(model_s.layer4[0].conv1.weight, stride=1) + deconv_orth_dist(
            model_s.layer4[0].conv3.weight, stride=1)
        ...
        loss = opt.gamma * loss_cls + opt.alpha * loss_div + opt.beta * loss_kd + 0.05*diff
        # print(diff)
