...
class MetaStanfordDogs(StanfordDogs):

    def __init__(self, args, partition='train', train_transform=None, test_transform=None, fix_seed=True):
        super(MetaStanfordDogs, self).__init__(args, partition, False)
        ...
        #Modified for Sanford Cars
        self.data = {}
        for idx in range(self.imgs.shape[0]):
            if self.labels[idx] not in self.data:
                self.data[self.labels[idx]] = []
            self.data[self.labels[idx]].append(self.imgs[idx])
        self.classes = self.labels[:]#list(self.data.keys())
...
if __name__ == '__main__':
    ...
    imagenet = MetaStanfordDogs(args, 'val')
    print(len(imagenet))
    print(imagenet.__getitem__(1)[0].shape)

    metaimagenet = MetaStanfordDogs(args)
    print(len(metaimagenet))
    print(metaimagenet.__getitem__(1)[0].size())
    print(metaimagenet.__getitem__(1)[1].shape)
    print(metaimagenet.__getitem__(1)[2].size())
    print(metaimagenet.__getitem__(1)[3].shape)
